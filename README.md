...
- **Entity Registry Seamless Migration**:
  ...
  - Built-in **Auto-Migration Script (r20/r21/r22)**: Seamlessly converts all legacy unique IDs (including those with legacy platform prefixes, custom diagnostic sensors, and orphaned notify actions) in the Home Assistant registry on startup. It guarantees **zero broken automations, Node-RED flows, or dashboards** by catching legacy orphaned entities and automatically migrating them.
...
## New Features & Enhancements (Version 20260531r24)
- **Technical Debt Phase II Cleanup**:
  - **Hardcoded English Error Messages Fixed (TD-07)**: Eliminated three instances in `options_flow.py` where `AbortFlow` was raised with a generic `storage_error` code and a raw English `error` placeholder string. Introduced three dedicated reason keys (`storage_error_net_config`, `storage_error_save_devices`, and `storage_error_user_config`) and localized them across all three supported translation files (`en.json`, `zh-Hant.json`, `zh-Hans.json`).
  - **Exception Handling Hardened (TD-02 / TD-06)**: Removed the class-level `# pylint: disable=broad-exception-caught` from `miot_client.py`'s class definition. Narrowed broad `except Exception` blocks inside key device polling helper methods `_get_from_cloud` and `_get_from_local` (both gateway and LAN pathways) to precise exception tuples `(MIoTClientError, MIoTHttpError, asyncio.TimeoutError)` and `(MIoTClientError, asyncio.TimeoutError)` respectively. Added point-to-point inline `# pylint: disable` annotations only where broad exceptions are structurally required.
